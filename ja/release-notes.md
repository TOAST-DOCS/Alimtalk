...
* [API] 알림톡 PC 노출/미노출 필드 변경
    * pcFlag -> securityFlag로 필드가 변경되었습니다. (default: false)
    * 보안 템플릿 여부를 나타내는 필드로, OTP등 보안 메시지 일 경우 설정하여야 합니다.
    * true로 등록될 경우, 메인 디바이스를 제외한 모든 디바이스에 메시지 텍스트 미노출됩니다.
* [API] 알림톡 채널 추가 버튼 타입 변경
    * 채널 추가 버튼 타입이 CA -> AC로 변경되었습니다.
    * 채널 추가 버튼은 버튼리스트 중, 최상단 혹은 단독에 있을 경우만 등록 가능합니다.
...
* [Console] 알림톡 발송 결과 백업 기능 추가
    ...
    * 콘솔에서 조회한 검색 조건을 기반으로 발송 결과 엑셀 파일을 생성할 수 있습니다.
    * 발송 결과 파일은 7일 이후 삭제됩니다.
* [Console] 친구톡 발송 결과 상세 조회 모달 개선
    * 친구톡 발송 결과 상세 조회 시, 광고 여부/대체 발송 요청 결과 추가
