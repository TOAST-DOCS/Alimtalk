...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (SMS/LMS再送信ステータス appears under 一般メッセージ/メッセージリストの照会 but has no ko counterpart at that location; k34 is a sibling of k28 under k27, not under k10) -->
#### SMS/LMS再送信ステータス
| 値 | 説明                      |
| ----- | ------------------------------- |
| RSC01 | 再送信の対象ではない                 |
...
```
curl -X GET \
https://kakaotalk-bizmessage.api.nhncloudservice.com/alimtalk/v2.2/appkeys/{appKey}/'"${APP_KEY}"'/mass-messages?requestId='"${REQUEST_ID}" \
...
```
curl -X GET \
https://kakaotalk-bizmessage.api.nhncloudservice.com/alimtalk/v2.2/appkeys/{appKey}/'"${APP_KEY}"'/mass-messages/recipients?requestId='"${REQUEST_ID}" \
-H 'Content-Type: application/json;charset=UTF-8' \
-H 'X-Secret-Key:{secretkey}'
```
...
```
curl -X GET \
https://kakaotalk-bizmessage.api.nhncloudservice.com/alimtalk/v2.2/appkeys/{appKey}/'"${APP_KEY}"'/mass-messages/recipients/1?requestId='"${REQUEST_ID}" \
-H 'Content-Type: application/json;charset=UTF-8' \
-H 'X-Secret-Key:{secretkey}'
```
...
|-- templateEmphasizeType| String| テンプレートのハイライトタイプ(NONE:基本、 TEXT:強調表示、 IMAGE:画像型)                                              |
