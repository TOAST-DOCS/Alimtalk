...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (SMS/LMS再送信ステータス appears under 一般メッセージ > メッセージリストの照会, but the ko counterpart k10 has no such subheading; k34 (SMS/LMS 대체 발송 상태 코드) belongs under k27 メッセージ section, not here) -->
#### SMS/LMS再送信ステータス
| 値 | 説明                      |
| ----- | ------------------------------- |
| RSC01 | 再送信の対象ではない                 |
...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (SMS/LMS再送信ステータス appears under 認証メッセージ > メッセージリストの照会, but the ko counterpart k21 has no such subheading) -->
<a id="response-7"></a>
