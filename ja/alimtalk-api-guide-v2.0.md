...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (SMS/LMS代替送信ステータスコードに相当するが、一般メッセージセクション内のL4として配置されており、ko側では一般メッセージ配下に対応する見出しが存在しない（k34はメッセージセクション配下のL3）) -->
#### SMS/LMS再送信ステータス
| 値 | 説明                      |
| ----- | ------------------------------- |
| RSC01 | 再送信の対象ではない                 |
...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (認証メッセージセクション内のL4 SMS/LMS再送信ステータス見出しで、ko側の対応する見出しは存在しない（k34はメッセージセクション配下のL3）) -->
<a id="status-of-resending-smslms"></a>
