...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (SMS/LMS再送信ステータスがko一般メッセージセクション内に存在するが、koにはこの位置に対応する見出しがない（k34はメッセージセクションに属する）) -->
#### SMS/LMS再送信ステータス
| 値 | 説明                      |
| ----- | ------------------------------- |
| RSC01 | 再送信の対象ではない                 |
...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (SMS/LMS再送信ステータスが認証メッセージセクション内に存在するが、koにはこの位置に対応する見出しがない) -->
<a id="status-of-resending-smslms"></a>
...
<a id="smslms-alternative-delivery-status-codes"></a>
