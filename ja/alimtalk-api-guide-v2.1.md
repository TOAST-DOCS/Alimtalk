...
| requestDate            | String  | X    | リクエスト日時(yyyy-MM-dd HH:mm)<br>(入力しない場合は即時送信)<br>最大30日以降まで予約可能 |
...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (SMS/LMS再送信ステータス appears under 一般メッセージ > メッセージリストの照会 but has no ko counterpart at that location; k34 (SMS/LMS 대체 발송 상태 코드) is an L3 heading under k27 メッセージ, not under k10) -->
#### SMS/LMS再送信ステータス
| 値 | 説明                      |
| ----- | ------------------------------- |
| RSC01 | 再送信の対象ではない                 |
...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (SMS/LMS再送信ステータス appears under 認証メッセージ > メッセージリストの照会 but has no ko counterpart at that location) -->
<a id="response-7"></a>
