## Notification > KakaoTalk Bizmessage > 친구톡 > 콘솔 사용 가이드

## 개요

* 카카오의 **친구톡 서비스**가 **2025년 12월 31일(수)** 을 마지막으로 종료합니다.
* 이에 따라 **NHN Cloud**에서도 **2025년 12월 30일(화)** 부로 친구톡 지원을 종료합니다.

|           | 내용 |
|:----------| :--- |
| 서비스 종료일   | 2025년 12월 30일(화) Notification 정기 배포 이후 |
| 영향 범위     | 발송을 비롯한 친구톡의 모든 서비스 중단 |
| 예외 지원 | 친구톡 호환 발송 기능만 한시적으로 제공<br>(발송 API, 발송 조회 API, 발송 결과 조회 콘솔) |

* 고객님의 원활한 브랜드 메시지 전환을 위해 다음 두 가지 전환 방식을 지원합니다.
    1. **(권장) 브랜드 메시지 자유형 발송 API로 전환**
    2. (임시) 기존 친구톡 API를 활용한 친구톡 호환 발송 기능

## 1\. \(권장\) **브랜드 메시지 자유형 발송 API로 전환**

* 보다 안정적이고 장기적인 발송을 위해 **브랜드 메시지 전용 API로 직접 전환**하는 것을 권장드립니다.

### 브랜드 메시지 전환의 이점

* 브랜드 메시지 자유형 발송은 I타겟팅 사용시 기존 친구톡 발송 API와 사용성이 동일하여 전환에 큰 어려움이 없습니다.
* 브랜드 메시지는 기존 친구톡에서 지원하는 기능을 모두 지원하는 상위 호환 서비스입니다.
* 브랜드 메시지는 타겟팅을 지정하여 발신 채널 친구가 아닌 고객에게도 광고 발송이 가능합니다.
* 자세한 내용은 브랜드 메시지 가이드({link})를 참고해주시기 바랍니다.

## 2\. \(임시\) 친구톡 호환 발송 기능

* 친구톡 호환 발송 기능은 친구톡 메시지 전문을 브랜드 메시지 포맷으로 전환하여 발송해주는 서비스입니다.

### 친구톡 호환 발송 기능 제공사항

| 항목 | 내용                                                           |
| :--- |:-------------------------------------------------------------|
| 지원 기간 | 약 1년간 (2025.12.30 \~ 2026.12.31 예정)                          |
| 지원 범위 | 기존 친구톡 일반 발송 API / 조회 API / 콘솔                               |
| 지원 내용 | 기존 친구톡 URI 및 JSON 포맷 그대로 사용 가능<br>응답 포맷 또한 기존 친구톡과 동일        |
| 내부 처리 방식 | 친구톡 전문을 카카오 브랜드 메시지의 **친구톡 호환 발송 API**로 전송                   |
| 검증 규칙 | 기존 친구톡 validation 규칙 유지<br>다만 발송 시 브랜드 메시지 스펙 적용으로 실패 가능성 존재 |

* 친구톡 호환 발송 기능은 2025년 10월 28일에 적용된 상태이며 현재 발송 테스트가 가능합니다.
* fade-out 이후 바로 전환 발송을 이용할 고객께서는 발송 테스트가 필요할 수 있습니다.
* 자세한 스펙 차이는 아래 표를 참고해주세요.

### 친구톡 호환 발송 이용 방법

* 기존 이용하시던 친구톡 API 요청에 헤더로 ```X-Convert-To-Brand-Message: true``` 값을 추가해주시면 친구톡 호환 발송 기능을 이용할 수 있습니다.
* 호환 발송이 잘 이루어졌는지 명시적으로 확인할 방법을 따로 제공하지는 않는 점 참고 부탁드립니다.
* 친구톡 호환 발송 기능은 API 버전과 무관하게 제공됩니다.

[Header]

```
{
  "X-Convert-To-Brand-Message": Boolean
}
```

| 이름 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| X-Convert-To-Brand-Message | Boolean | X | 친구톡 호환 발송을 위한 요청 헤더 (2025년 12월 30일 부터 ```X-Convert-To-Brand-Message: true``` 헤더가 없는 값은 요청 단계에서 발송 실패 처리됩니다.) |

### 카카오 제공 스펙 변경 비교표

![friendtalk_compatible_spec_01.png](https://static.toastoven.net/prod_alimtalk/friendtalk_compatible_spec_01.png)

### 그 외의 확인된 스펙 차이

* 캐러셀 피드
    * 각 캐러셀 당 버튼 옵셔널 -> 각 캐러셀 당 버튼 필수
* 캐러셀 커머스
    * 최소 1장 (인트로 포함시) \~ 최대 10장 -> 최소 2장 \~ 최대 6장 (인트로 포함)</span>
* 이 밖에 확인되지 않은 스펙 차이가 있을 수 있는 점 참고 부탁드립니다.