...
### 블랙리스트
어뷰징을 방지하기 위해 특정 기준을 충족하지 못한 발신 프로필은 블랙리스트에 등록되어 일부 기능에 제약을 받게 됩니다.

#### 블랙리스트 제약 사항
1. 일별 발송량 제한
2. 그룹 프로필에 멤버로 추가 불가
3. 템플릿 변수가 치환될 때 그 차이가 14글자를 초과하는 부분이 있는 경우, 메세지 발송 실패 처리

#### 블랙리스트 해제 기준
1. 발신 프로필 신규 등록시 같은 프로젝트 내에 이미 블랙리스트가 해제된 발신 프로필이 존재하는 경우 자동으로 블랙리스트가 해제됩니다.
2. 블랙리스트에 등록된 발신 프로필 생성 이후 1달 이내에 해당 프로필로 과금 정상 발송 템플릿 건 수가 10건 이상인 경우, 다음날 자동으로 블랙리스트가 해제됩니다.
3. 이외에 블랙리스트 해제가 필요한 경우 별도로 문의바랍니다.