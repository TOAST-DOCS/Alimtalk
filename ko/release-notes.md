...
## Notification > KakaoTalk Bizmessage > 릴리스 노트 { #notification-kakaotalk-bizmessage-release-notes }
...
* [API] 브랜드 메시지 080 수신거부 인증 번호 최대 길이 변경
    * 080 수신거부 인증 번호의 최대 허용 길이가 9자에서 10자로 변경되었습니다.
...
#### 기능 개선/변경
* [Console] 브랜드 메시지 비디오 업로드 기능 추가
    * 브랜드 메시지 프리미엄 비디오에 사용할 동영상을 콘솔에서 직접 업로드하고 관리할 수 있습니다.
* [API] 브랜드 메시지 비디오 업로드 API 추가
    * 브랜드 메시지 프리미엄 비디오용 동영상을 업로드할 수 있는 API가 추가되었습니다.
* [API] 브랜드 메시지 결과 수신 웹훅에 채널 친구 여부 필드 추가
    * 브랜드 메시지 결과 수신 웹훅 응답에 수신자의 카카오 채널 친구 여부를 나타내는 `isAddedChannel`(Boolean) 필드가 추가되었습니다.

<a id="bug-fixes"></a>
#### 버그 수정
* [API] 브랜드 메시지·친구톡 광고 대체발송 시 080 수신거부 번호 처리 개선
    * 브랜드 메시지·친구톡 광고 대체발송 시 080 수신거부 번호가 없는 일부 케이스에서 일반 메시지로 발송되던 문제를 수정했습니다.

<a id="may-27-2026"></a>
### 2026. 05. 27. { #may-27-2026 }
* [API] 카카오비즈센터 통계 조회 API 추가
    * 카카오비즈센터에서 제공하는 알림톡, 브랜드 메시지의 발송 통계 및 템플릿 통계를 조회할 수 있는 API가 추가되었습니다.
    * 발신 키 기준으로 일별(DAILY) 또는 월별(MONTHLY) 통계 데이터를 조회할 수 있습니다.

* [API, 콘솔] 브랜드 메시지 이미지 삭제 관련 가이드 수정
    * 브랜드 메시지 템플릿 수정 시 이미지를 변경하면 기존 이미지가 카카오 CDN에서 삭제되며, 동일한 이미지를 사용하는 다른 템플릿도 영향을 받는다는 내용으로 가이드가 수정되었습니다.

* [API, 콘솔] 브랜드 메시지 유형별 내용 길이 제한 변경
    * IMAGE 타입의 메시지 내용(content) 길이 제한이 최대 400자에서 최대 1,300자로 변경되었습니다.
    * IMAGE 타입의 줄바꿈 제한이 최대 29개에서 최대 99개로 변경되었습니다.
    * WIDE, PREMIUM_VIDEO 타입의 줄바꿈 제한이 최대 1개에서 최대 5개로 변경되었습니다.
...
* [API] 휴면 후 삭제된 발신 프로필, 템플릿 데이터 자동 정리
    * 카카오에서 휴면 후 1년이 지나 삭제 처리된 발신 프로필과 템플릿 데이터를 동기화하여 자동 정리합니다.
* [API] 브랜드 메시지 자유형 발송 채널 추가 버튼(AC 타입) 지원
    * 자유형 발송의 채널 추가 버튼을 모든 타겟팅에서 사용할 수 있도록 확장했습니다.
    * 수신자가 친구인 경우 발송이 실패하진 않으나 채널 추가 버튼이 노출되지 않습니다.

<a id="april-28-2026-2"></a>
### 2026. 04. 28. { #april-28-2026-2 }
* [API] 휴면 후 삭제된 발신프로필, 템플릿 데이터 자동 정리
    * 카카오에서 휴면 후 1년이 지나 삭제 처리된 발신프로필과 템플릿 데이터를 동기화하여 자동 정리합니다.
* [API] 브랜드 메시지 자유형 발송 채널 추가 버튼(AC 타입) 지원
    * 자유형 발송의 채널 추가 버튼을 모든 타겟팅에서 사용할 수 있도록 확장했습니다.
    * 수신자가 친구인 경우 발송이 실패하진 않으나 채널 추가 버튼이 노출되지 않습니다. 
...
* [콘솔] 카카오비즈센터 통계 제공
    * 카카오비즈센터에서 제공하는 알림톡, 브랜드 메시지 통계 데이터를 조회할 수 있는 기능을 제공합니다.

* [콘솔] 카카오비즈센터 그룹 태그 관리 기능 제공
    * 카카오비즈센터에서 제공하는 그룹 태그를 관리할 수 있는 기능을 제공합니다.

* [콘솔]
    * 발송 이력 백업 시 결과에 대한 알림 메일을 발송합니다.
    * 수신 대상은 프로젝트 대시보드 > 알림 관리 탭에서 설정할 수 있습니다.
...
* [API] 브랜드 메시지 캐러셀 고정 치환자 기능 추가
    * 캐러셀 타입 템플릿에서 각 아이템에 다른 치환자 값을 적용할 수 있습니다.
    * path 기반 파라미터(`키@$.carousel.list[인덱스]`)를 통해 아이템별 개인화가 가능합니다.
    * 템플릿 치환자 키에 '@' 문자를 사용할 수 없습니다.

* [API] 브랜드 메시지 템플릿 치환자 키 유효성 검사 추가
    * 템플릿 등록/수정 시 치환자 키에 `@` 문자가 포함되면 오류가 발생합니다.

* [API] 브랜드 메시지 발송 시 템플릿 파라미터 누락 검사 추가
    * 발송 요청 시 템플릿에 정의된 치환자 키가 templateParameter에 누락되면 오류가 발생합니다.

* [API, 콘솔] 브랜드 메시지 BT 버튼 타입 지원
    * 브랜드 메시지에서 BT(봇 전환) 버튼 타입을 지원합니다.
    * 콘솔에서 BT 버튼 타입을 선택하여 템플릿을 등록할 수 있습니다.

* [API, 콘솔] 브랜드 메시지 AC 버튼 타입 추가
    * 브랜드 메시지에서 AC(채널 추가) 버튼 타입을 지원합니다.
    * AC 버튼의 이름은 반드시 '채널 추가'여야 합니다.
    * 캐러셀의 경우 전체 아이템 중 AC 버튼은 1개만 사용 가능합니다.

* [콘솔] 알림톡/브랜드 메시지 대체 발송 응답 메시지 분류 개선
    * 대체 발송 시 응답 메시지가 분류되어 표시됩니다.

* [API] 템플릿 파라미터 길이 제한 추가
    * 템플릿 파라미터의 키와 값은 각각 최대 1,300자로 제한됩니다.

* [API] 수신자 그룹핑 키 길이 제한 추가
    * recipientGroupingKey 필드의 최대 길이가 100자로 제한됩니다.

<a id="january-27-2026"></a>
### 2026. 01. 27. { #january-27-2026 }
* [API] 알림톡 예약/대량 발송 결과 웹훅에 조회 링크 추가
    * 예약 발송과 대량 발송 결과 웹훅에 발송 결과 조회 API 링크가 추가되었습니다.

<a id="december-31-2025"></a>
### 2025. 12. 31. { #december-31-2025 }
* [API, 콘솔] 친구톡 서비스 지원 종료
    * 2025년 12월 31일(수) 친구톡 서비스가 종료됩니다.
    * 친구톡 API에서는 호환 헤더 설정을 통한 브랜드 메시지 전환 기능을 한시적으로 지원합니다.
    * 친구톡 콘솔에서는 발송 결과 조회, 이미지 관리, 대체 발송 관리 탭만 사용 가능합니다.

* [API] 알림톡 템플릿 단건 조회 API 추가
    * 특정 템플릿을 단건으로 조회할 수 있는 API가 추가되었습니다.
    * 템플릿의 차단, 휴면 여부를 확인할 수 있는 필드가 응답에 포함됩니다.

<a id="november-25-2025"></a>
### 2025. 11. 25. { #november-25-2025 }

* [콘솔] 브랜드 메시지 확인 후 발송 기능 추가
    * 브랜드 메시지 대량 발송 시 발송 전 확인 후 발송할 수 있는 기능이 추가되었습니다.
    * 검수 후 발송을 통해 대량 발송 전 최종 확인이 가능합니다.

* [콘솔] 브랜드 메시지 대량 발송 이미지 개인화 필드 입력 개선
    * 브랜드 메시지 대량 발송의 수신자 업로드 파일 내의 이미지 개인화 필드 형식을 개선하였습니다.
    * 이전에 다운로드한 수신자 업로드 Excel, CSV 파일은 유효하지 않을 수 있습니다.

* [API, 콘솔] 브랜드 메시지 개인화 필드 유효성 검증 개선
    * 브랜드 메시지 개인화 필드의 유효성 검증이 카카오 스펙에 맞게 개선되었습니다.
    * 개인화 필드 입력 시 보다 정확한 검증을 통해 발송 실패를 방지할 수 있습니다.

* [콘솔] 브랜드 메시지 링크 필드 스펙 변경
    * 브랜드 메시지 자유형 메시지의 링크 필드 스펙은 1,000자, 템플릿 링크 필드 스펙은 500자로 카카오 스펙에 맞춰 변경하였습니다.
    * 변경된 스펙에 맞게 링크 필드를 설정하여 발송할 수 있습니다.

* [API] 브랜드 메시지 대체 발송 동작 개선
    * 브랜드 메시지 발송 시 resendParameter를 입력하지 않으면 기본 설정에 따라 대체 발송이 동작하도록 개선되었습니다.

* [API] 알림톡 결과 조회 count API 추가
    * 결과 조회 API와 동일한 조건으로 count 수를 응답하는 API가 추가되었습니다.

<a id="october-28-2025"></a>
### 2025. 10. 28. { #october-28-2025 }
* [API, 콘솔] 알림톡 템플릿 유효성 변경
    * 알림톡 템플릿의 최대 글자수 제한이 1,000자에서 1,300자로 확대되어 더 긴 메시지를 발송할 수 있습니다.

* [API, 콘솔] 브랜드 메시지 야간 광고 재발송 기능
    * 브랜드 메시지에서 야간 광고 재발송 기능을 사용할 수 있습니다.
    * 야간(오후 8시 50분~오전 8시)에 광고성 메시지 발송 시 자동으로 익일 오전 8시에 재발송됩니다.
    * 콘솔에서 야간 광고 재발송 기능을 설정할 수 있습니다.

<a id="september-23-2025"></a>
### 2025. 09. 23. { #september-23-2025 }

* [API] 브랜드 메시지 video 파라미터 개인화 기능 추가
    * 브랜드 메시지 발송 시 videoParameter 필드를 통해 동적으로 템플릿의 video 값을 변경하여 발송하는 기능이 추가되었습니다.
    * 수신자별로 다른 동영상을 지정하여 맞춤형 메시지 발송이 가능합니다.

* [API] 브랜드 메시지 캐러셀 이미지 개인화 적용
    * 카카오 측 버그 수정으로 캐러셀 피드형, 캐러셀 커머스형 타입에도 이미지 개인화 기능이 적용됩니다.
    * 캐러셀 타입 메시지에서도 수신자별로 다른 이미지를 지정할 수 있습니다.

* [API] 알림톡 전화하기 버튼 유형 추가
    * 알림톡 템플릿에 전화하기 버튼 타입이 추가되었습니다.
    * 사용자가 버튼 클릭 시 바로 전화를 걸 수 있는 기능을 제공합니다.

* [콘솔] 브랜드 메시지 Excel 다운로드 기능 추가
    * 브랜드 메시지 발송 결과 조회 화면에서 Excel 다운로드 기능이 추가되었습니다.
    * 발송 이력을 Excel 파일로 다운로드하여 관리할 수 있습니다.

<a id="august-26-2025"></a>
### 2025. 08. 26. { #august-26-2025 }

* [API, 콘솔] 브랜드 메시지 - 자유형 발송 M/N 타입 기능 지원
    * 브랜드 메시지에서 자유형 메시지 M/N 타입 발송이 가능하도록 기능이 추가됩니다.
    * 기존 기본형에 추가하여 자유형 M/N 타입으로 브랜드 메시지를 보다 유연하게 발송할 수 있습니다.

* [API] 브랜드 메시지 - 재판매사 식별코드 발송 API 필드 추가
    * 브랜드 메시지 발송 API에 재판매사 식별코드를 지정할 수 있는 필드가 추가됩니다.
    * 재판매사별 메시지 발송을 구분하여 관리할 수 있는 기능이 제공됩니다.

* [API, 콘솔] 브랜드 메시지 기능 추가 - 예약 발송
    * 브랜드 메시지 서비스에 예약 발송 기능이 추가됩니다.
    * 원하는 시간에 브랜드 메시지를 예약하여 발송할 수 있습니다.

* [API] 브랜드 메시지 - senderGroupingKey, recipientGroupingKey 추가
    * 브랜드 메시지 발송 API에 발신자 그룹핑 키와 수신자 그룹핑 키 필드가 추가됩니다.
    * 메시지 발송 요청을 그룹별로 관리하여 발송 결과를 효율적으로 추적할 수 있습니다.

<a id="july-29-2025"></a>
### 2025. 07. 29. { #july-29-2025 }
* [API, 콘솔] 친구톡 Upgrade → 브랜드 메시지 이름 변경
...
* [API, 콘솔] 브랜드 메시지 M/N 타겟팅 사용 신청 기능 추가
...
* [API, 콘솔] 발신 프로필 080 무료수신거부 정보 등록 API 신규 추가
    * 브랜드 메시지 발송 시 사용되는 발신 프로필의 080 무료수신거부 정보를 등록할 수 있습니다.
    * 080 무료수신거부 정보 등록 시 수정은 가능하나 빈 값으로 초기화가 불가능합니다.

* [API] 브랜드 메시지 기본형 발송 시 수신자별로 템플릿 이미지 필드를 변경하는 기능 추가
    * 브랜드 메시지 기본형 발송 시 수신자별로 템플릿 이미지 필드를 변경할 수 있는 imageParameters 필드가 추가되었습니다.
        * 기본형 발송 시 imageParameters 필드를 통해 각 이미지 필드의 imageUrl, imageLink 의 값을 대체할 수 있습니다.

* [콘솔] 브랜드 메시지 080 수신거부 관리 초기화 버그 수정
    * SMS 서비스 앱키를 등록한 경우 초기화가 안 되는 버그를 수정하였습니다.
    * SMS 서비스 앱키 초기화 시 기존에 등록한 대체 발송 설정이 삭제됩니다.

<a id="june-24-2025"></a>
### 2025. 06. 24. { #june-24-2025 }
* [API, 콘솔] 브랜드 메시지 웹훅 기능 추가
    * 발송 결과 업데이트 시, 웹훅 기능이 추가되었습니다.
* [API, 콘솔] 브랜드 메시지 대체 발송 기능 추가
    * 브랜드 메시지 발송 시 대체 발송 기능이 추가되었습니다.
    * '발송 설정'에서 SMS 서비스 앱키를 등록하면, 브랜드 메시지 발송 시 대체 발송이 가능합니다.
        * 입력한 발신 번호가 SMS 상품에 등록된 발신 번호가 아닐 경우, 대체 발송이 실패할 수 있습니다.
* [API] 브랜드 메시지 메시지 타입 텍스트형 유효성 변경
    * 텍스트형 본문 길이 제한 기준이 변경되었습니다.
    * 변경 사항
        * 본문 길이 제한: 1,000자 → 1,300자
        * 줄바꿈: 33줄 → 99줄
* [API, 콘솔] 브랜드 메시지 템플릿 발송 타겟팅 기능 개선
    * 브랜드 메시지 템플릿 발송 시, 전체 메시지 유형에 대해 M, N 타입 발송이 가능하도록 개선됩니다.
* [API, 콘솔] 브랜드 메시지 템플릿 발송 080 수신거부번호 필드 추가
...
* [콘솔] 알림톡 템플릿 조회 탭에 템플릿 차단 여부 노출
    * 알림톡 템플릿 조회 탭에 템플릿 차단 여부가 노출됩니다.
* [콘솔] 브랜드 메시지 대량 발송 기능 추가
    * CSV, XLSX 파일을 이용한 대량 발송 기능이 추가되었습니다.
* [콘솔] 브랜드 메시지 발송 통계 화면 제공
    * 날짜별/시간대별/요일별 통계 화면이 제공됩니다.

<a id="may-13-2025"></a>
### 2025. 05. 13. { #may-13-2025 }
* [API] 브랜드 메시지 서비스 베타 출시
    * 브랜드 메시지 기본형 M, N 그룹(마케팅 수신동의 회원 타깃) 발송 가능 대상: 아래 조건에 부합하는 경우 화이트리스트 등록 후 발송 가능합니다(카카오 승인 필요).
...
* [API] 카카오톡에서 메시지를 발송할 수 없을 경우 반환되던 응답 코드(3018)가 아래와 같이 세분화되어 제공 예정
...
* [콘솔] 수신 화면 미리보기 탭에서 강조 표기형 템플릿의 일부 필드가 치환되지 않는 버그 수정
...
* [콘솔] 예약 발송 최대 예약 가능 기간 변경
    * 예약 발송의 발송 일시를 현재로부터 최대 60일 이후까지 설정이 가능하도록 변경되었습니다.
* [콘솔] 알림톡 아이템리스트형 요약 필드가 초기화되지 않는 버그 수정
    * 알림톡 아이템리스트형 요약 필드 입력 후, 삭제 시 초기화되지 않는 버그를 수정하였습니다.
* [콘솔] 알림톡 템플릿 등록 시 불필요한 필드 정보가 등록되는 버그 수정
...
* [콘솔] 역할 세분화
    * 역할에 따라 KakaoTalk Bizmessage 메뉴 접근 및 기능 제어 권한이 별도로 부여되도록 기능이 추가되었습니다.
    * 역할 설정 방법은 [콘솔 사용 가이드 > 멤버 관리 > 프로젝트 멤버](https://docs.nhncloud.com/ko/nhncloud/ko/console-guide/#_24) 단락을 참고하세요.
...
* [API] 친구톡 버튼 링크형 필드의 길이 제한 변경(500자 → 2,000자)
    * 친구톡 버튼의 PC 링크 필드, 모바일 링크 필드 등의 최대 길이 제한을 2,000자로 변경하였습니다.
* [API] 친구톡 이미지 파일 사이즈 제한 변경(2MB → 5MB)
    * 친구톡 이미지의 최대 파일 제한을 5MB로 변경하였습니다.

<a id="february-27-2024"></a>
### 2024. 02. 27. { #february-27-2024 }
* [콘솔] 알림톡 템플릿 등록 시 문의 및 첨부 파일 등록 가능
...
* [API] 친구톡 와이드 아이템리스트 제목 글자 수 제한 변경
    * (변경 전) 친구톡 와이드 아이템리스트 발송 시 1~4번째 아이템 제목 글자 수는 25자 제한
    * (변경 후) 친구톡 와이드 아이템리스트 발송 시 첫 번째 아이템 제목 글자 수는 25자 제한, 2~4번째 아이템 제목 글자수는 30자 제한
...
* [콘솔] 알림톡 대량 발송 시, 과거 일자로 요청되던 버그 수정
...
* [콘솔] 발신 프로필 최초 사용자 제한
    * 카카오 정책으로 어뷰징 행위 방지를 위해 최초 등록 발신 프로필에 대한 제약이 추가되었습니다.
        1. 그룹 프로필에 멤버로 추가 불가
        2. 템플릿 변수가 치환될 때, 그 차이가 14글자를 초과하는 경우, 메시지 발송 실패 처리

<a id="october-25-2022"></a>
### 2022. 10. 25. { #october-25-2022 }
* [API] 발신 프로필 조회 API isSearchKakaoStatus 필드 삭제
    * 발신 프로필 조회 API에서 isSearchKakaoStatus 필드를 삭제하였고, 준실시간 카카오 상태 조회가 가능하도록 개선하였습니다.(6시간 주기로 상태 동기화)
...
* [콘솔] 알림톡 템플릿 채널추가형(AD)과 복합형(MI) 메시지 유형의 templateAd 일괄 변경
    * 카카오 정책에 따라, "채널 추가하고 이 채널의 광고와 마케팅 메시지를 카카오톡으로 받기" 로 일괄 변경됩니다.
...
* [API] 알림톡 템플릿 등록/수정 시 templateAd 필드 삭제
    * 알림톡 템플릿 등록/수정 시 templateAd 필드가 삭제되었습니다.
    * 채널 추가형(AD) 또는 복합형(MI) 메시지 유형 템플릿 등록 시 templateAd 값이 고정됩니다.

<a id="june-14-2022"></a>
### 2022. 06. 14. { #june-14-2022 }
* [콘솔] 채널 추가형/복합형 템플릿 변경 사항
    * 카카오톡 비즈메시지 정책 변경으로, 채널 추가형/복합형 템플릿인 경우, 채널 추가 버튼과 채널 추가 안내 메시지가 고정됩니다.

<a id="may-24-2022"></a>
### 2022. 05. 24. { #may-24-2022 }
* [API] 알림톡 템플릿 삭제 개선
    * 반려된 템플릿 이외에 다른 상태의 템플릿도 삭제가 가능하도록 개선되었습니다.
    * 자세한 내용은 [API 가이드](./alimtalk-api-guide/#_50)를 참고하세요.
...
* [콘솔] 알림톡 발송 결과 백업 기준일 변경
    * 알림톡 발송 결과 백업 기준일이 180일에서 90일로 변경되었습니다.
* [콘솔] 일반, 대량, 태그 발송 결과 다운로드 기능 개선
...
* [콘솔] 발신 프로필 그룹의 멤버 최대 5,000건 제한
    * 카카오 정책에 따라 발신 프로필 그룹의 최대 멤버 수를 5,000건으로 제한합니다.
* [콘솔] CloudTrail 적용
...
* [콘솔] 알림톡 180일이 지난 알림톡 발송 목록 데이터 백업 기능 추가
    * 180일이 지난 발송 목록(일반/대량)에 대해 고객의 오브젝트 스토리지 또는 AWS S3로 백업 파일을 생성하는 기능이 추가되었습니다.
...
* [API] 알림톡 템플릿 메시지 유형 확대
    * 알림톡 템플릿 메시지 유형이 확대되었습니다(BA: 기본형, EX: 부가 정보형, AD: 채널 추가형, MI: 복합형).
...
* [콘솔] 플러스친구 등록 시, 비즈니스 인증 방식 롤백
    * 플러스친구의 비즈니스 인증이 장시간 소요되어, 플러스친구 등록 방식을 이전 방식으로 변경하였습니다.

<a id="april-23-2019"></a>
### 2019. 04. 23. { #april-23-2019 }
* [콘솔] 알림톡 대량 발송 시, **대체 발송 지정 기능, 분할 발송 기능** 추가
    * 대체 발송 내용/발신 번호/대체 발송 여부 등 대체 발송 관련 필드를 지정할 수 있는 기능이 추가되었습니다.
    * 분할 횟수/시간 간격을 지정하여 분할 발송할 수 있는 기능이 추가되었습니다.
* [콘솔] 알림톡/친구톡 **대체 발송 설정 분리**
    * 알림톡/친구톡 각각의 플러스친구에 대체 발송 설정을 할 수 있는 기능이 추가되었습니다.
      예: 동일한 플러스친구에서 알림톡만 대체 발송 설정 시, 친구톡은 발송 실패하여도 대체 발송되지 않습니다.
* [콘솔] 플러스친구 등록 시, **비즈니스 인증 도입**
    * 플러스친구 등록 시 플러스친구의 **비즈니스 인증**된 경우만 등록 가능하도록 변경됩니다.​ [관련 공지사항](https://center-pf.kakao.com/notices/311)
* [콘솔] 템플릿 선택 모달 창에서 like 검색 기능 추가
    * 템플릿 선택 모달 창에서 손쉽게 템플릿을 선택할 수 있도록 like 검색 기능이 추가되었습니다.(템플릿코드, 템플릿명으로 검색)
* [API] 광고 친구톡, **광고 SMS API로 대체 발송** 되도록 개선
    * 광고 친구톡 발송 실패 시, 광고 SMS API로 대체 발송되도록 개선되었습니다.
    * 광고 친구톡 대체 발송 설정 시, 080 수신거부번호를 반드시 등록해야 정상적으로 대체 발송됩니다.
* [API] 친구톡 발송 API 대체 발송 고도화
    * 발송 시, 대체 발송 제목 필드가 추가되었습니다.
    * 대체 발송 필드를 이용하여, 대체 발송될 제목/내용/발신번호/080 수신거부번호/대체 발송 유무를 선택할 수 있습니다.
...
* [콘솔] 알림톡 대량 발송 시, 발송 실패 버그 수정
    * 일부 유효하지 않은 수신번호로 인해, 발송 실패되는 버그를 수정하였습니다.
* [콘솔] 친구톡 대량 발송 수신번호 마스킹되는 버그 수정
...
* [API] 친구톡 v1.2 API 추가
    ...
    * 발송 응답 필드에 각 수신자별 **요청 성공/실패** 필드가 추가되었습니다.
...
* [API] 알림톡 발송 결과 업데이트 조회 API 추가
    * 결과 업데이트 시간으로 조회할 수 있는 신규 API가 추가되었습니다.
* [API] 발송 시, 대체 발송 발신 번호 필드 추가
    * 발송 시, resendSendNo 필드를 통하여 대체 발송 발신 번호를 지정할 수 있는 기능이 추가되었습니다.
...
* [API] 알림톡 발송 API 대체 발송 고도화
    ...
    * 템플릿 본문 불일치(-3010), 템플릿 버튼 불일치(-3011)으로 요청 실패할 경우, 대체 발송되도록 변경되었습니다. 대체 발송이 필요없는 메시지는 isResend 필드를 이용하여, 조작 가능합니다.
* [API] 예약 발송 버그 수정
...
* [API] 알림톡 발송 API 예약 기능 추가
    ...
    * 자세한 사항은 [알림톡 발송 API](./alimtalk-api-guide/#_3)를 참고하세요.
* [API] 알림톡 발송 API 대체 발송 고도화
    * 발송 시, 대체 발송 타입(SMS/LMS), 대체 발송 여부(true/false), 대체 발송 내용 필드가 추가되었습니다.
...
* [콘솔] 템플릿 코드 Like 검색 기능 추가
    * 콘솔에서 템플릿 조회 시, Like 검색 기능이 추가되었습니다.
...
* [콘솔] 서비스명 변경
    * AlimTalk에서 KakaoTalkBizmessage로 서비스명이 변경되었습니다.
* [콘솔] 친구톡 기능 추가
    * 친구톡은 친구를 맺은 이용자를 대상으로 하며, 광고성 메시지도 발송이 가능합니다.
    * 콘솔에서 메시지 발송, 조회, 이미지 관리, 통계 기능을 제공합니다.
    * 자세한 사항은 [친구톡 개요](./friendtalk-overview/)를 참고하세요.
* [API] 친구톡 API 추가
    * 친구톡 API는 메시지 발송, 목록 조회, 단건 조회, 이미지 관리 기능을 지원합니다.
    * 자세한 사항은 [친구톡 API 가이드](./friendtalk-api-guide/)를 참고하세요.
* [API] 알림톡, 친구톡 일별 발송 제한 추가
    * 7월 24일 점검 이후, 생성된 플러스 친구에 한하여, 일별 1,000건의 발송량 제한 기능이 추가되었습니다.
...
#### 기능 개선
* [콘솔] 플러스친구 등록 시, 요청 필드 추가
    * 카카오 발급절차 강화로 인해 사업자등록번호, 사업자 카테고리를 입력받도록 수정되었습니다.

<a id="bug-fixes-2"></a>
#### 버그 수정
* [콘솔] 통계 차트 버그 개선 버전으로 변경
    * IE 브라우저에서 Export xls 기능 버그 개선 버전으로 변경하였습니다.
...
* [API] 발송 결과 단건 조회 API 추가
    ...
    * 자세한 사항은 [알림톡 API 가이드](./alimtalk-api-guide/#_14)를 참고하세요.
...
* [콘솔] 템플릿 챗버블 기능 추가
    ...
    * 버튼 타입 추가되었습니다.( 배송 조회, 웹 링크, 앱 링크, 봇 키워드, 메시지 전달 )
* [API] 템플릿 조회 API 추가
    * 템플릿을 조회할 수 있는 API가 추가되었습니다.
    * 자세한 사항은 [알림톡 API 가이드](./alimtalk-api-guide/#_46)를 참고하세요.
...
#### 기능 추가
* [콘솔] 템플릿 수정, 삭제, 문의 등록 기능 추가
    * 템플릿 수정, 삭제 기능이 추가되었습니다.
    * 검수 담당자에게 문의를 등록 기능이 추가되었습니다.
    * 자세한 사항은 [알림톡 콘솔 사용 가이드](./alimtalk-console-guide/#_12)를 참고하세요.
* [API] 전문 발송 API 추가
    * 치환 데이터가 아닌 전문 내용으로 발송할 수 있는 API가 추가되었습니다.
    * 자세한 사항은 [알림톡 API 가이드](./alimtalk-api-guide/#_4)를 참고하세요.
...
* [콘솔] 템플릿 반려 시, 템플릿 상세보기에서 반려 사유 제공.
...
* [API] Content-type 에러 응답 메시지 수정
    * 요청 header에 Content-type: application/json이 아닐 경우 실패 응답 메시지 수정되었습니다.
...
#### 기능 추가
* [콘솔] 알림톡 발송 결과 조회 추가
    * 발신 일시, 수신번호, 템플릿 등의 조건으로 발신한 메시지의 전송 결과를 조회할 수 있습니다.
