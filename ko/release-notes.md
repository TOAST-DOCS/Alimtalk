...
* [API] 브랜드 메시지 캐러셀 고정 치환자 기능 추가
  ...
  * 템플릿 치환키에 '@' 문자를 사용할 수 없습니다.

* [API] 브랜드 메시지 템플릿 치환자 키 유효성 검사 추가
  * 템플릿 등록/수정 시 치환자 키에 `@` 문자가 포함되면 에러가 발생합니다.

* [API] 브랜드 메시지 발송 시 템플릿 파라미터 누락 검사 추가
  * 발송 요청 시 템플릿에 정의된 치환자 키가 templateParameter에 누락되면 에러가 발생합니다.
...
* [Console] 알림톡/브랜드 메시지 대체발송 응답메시지 분류 개선
  * 대체발송 시 응답메시지가 분류되어 표시됩니다.
