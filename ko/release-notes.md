...
* [API] 브랜드 메시지 캐러셀 고정 치환자 기능 추가
  * 캐러셀 타입 템플릿에서 각 아이템별로 다른 치환자 값을 적용할 수 있는 기능이 추가되었습니다.
...
* [API, Console] 브랜드 메시지 BC/BT 버튼 타입 지원
  * 브랜드 메시지에서 BC(상담톡 전환), BT(봇 전환) 버튼 타입을 지원합니다.
  * 콘솔에서 BC, BT 버튼 타입을 선택하여 템플릿을 등록할 수 있습니다.

* [API, Console] 브랜드 메시지 AC 버튼 타입 추가
  * 브랜드 메시지에서 AC(채널 추가) 버튼 타입을 지원합니다.
  * AC 버튼의 이름은 반드시 '채널 추가'여야 합니다.
  * 캐러셀의 경우 전체 아이템 중 AC 버튼은 1개만 사용 가능합니다.

* [Console] 알림톡/브랜드 메시지 대체 발송 응답 메시지 분류 개선
  * 대체 발송 시 응답 메시지가 분류되어 표시됩니다.

* [API] 템플릿 파라미터 길이 제한 추가
  * 템플릿 파라미터의 키와 값은 각각 최대 1300자로 제한됩니다.
...
* [API, Console] 브랜드 메시지 기능 추가 - 예약 발송
  * 브랜드 메시지 서비스에 예약 발송 기능이 추가됩니다.
  * 원하는 시간에 브랜드 메시지 메시지를 예약하여 발송할 수 있습니다.
...
### 2023. 06. 27.
* [Console] 발신 프로필 조회 시, 신규 필드 추가(v2.3)
...
* [Console] 발신 프로필 그룹의 멤버 최대 5,000건 제한
...
* [API] 광고 친구톡, <b>광고 SMS API로 대체 발송</b> 되도록 개선
    ...
    - 광고 친구톡 대체 발송 설정 시, 080 수신거부번호를 반드시 등록해야 정상적으로 대체 발송 됩니다.
* [API] 친구톡 발송 API 대체 발송 고도화
    - 발송 시, 대체 발송 제목 필드가 추가 되었습니다.
    - 대체 발송 필드를 이용하여, 대체 발송 될 제목/내용/발신번호/080 수신거부번호/대체 발송 유무를 선택할 수 있습니다.
