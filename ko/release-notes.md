...
### 2024. 05. 28.
* [API] 특정 필드에 취소선 서체 스타일 사용 가능
    * 알림톡 발송시 강조 표기형 템플릿의 템플릿 제목과 아이템리스트형 템플릿의 아이템 하이라이트 제목의 경우 뒤에 동적으로 `\s` 문자를 추가할 경우 취소선이 적용됩니다.
    * 단, 템플릿 등록시 미리 `\s` 문자를 필드에 추가해놓는 경우에는 적용되지 않습니다.
