...
6. 하단 일반 발송탭에서 수신자 추가 버튼을 클릭하여 템플릿에 존재하는 치환키값을 입력한 후 수신자 번호를 입력합니다.
...
* 템플릿을 선택 후, <b>템플릿 다운로드</b> 버튼을 클릭 시 템플릿 치환자가 포함된 CSV, XLSX 파일을 다운로드할 수 있습니다. 템플릿 치환자가 포함되지 않을 경우 발송 실패처리 됩니다.
...
* 확인 후 진행 : <b>대량 발송 조회</b 탭에서 7일 이내에 메일 수신자 확인 후 알림톡 발송을 진행합니다. (예약 발송 시 지원하지 않습니다.)
* 즉시 발송 : 알림톡 수신자를 확인하지 않고 즉시 발송을 진행합니다. 발송 상태는 <b>대량 발송 조회</b> 탭에서 결과를 확인할 수 있습니다.


### 대체 발송
알림톡 발송이 실패될 경우, 해당 내용을 문자메시지로 대체 발송할 수 있는 기능입니다.
<b>공통된 내용으로 발송, 사용자별 다른 메시지 설정</b> 2가지를 선택하여 발송할 수 있습니다.
![KTB_12_20230926.png](https://static.toastoven.net/prod_alimtalk/KTB_12_20230926.png)
* [대체 발송 관리] 탭에서 대체 발송 여부를 ‘사용’으로 설정 및 SMS 서비스를 이용해야 대체 발송이 가능합니다.
...
#### 공통된 내용으로 발송
* <b>발신 프로필/템플릿</b>을 선택 후 <b>공통된 내용으로 발송</b> 버튼을 클릭하여 대체 발송 메시지를 설정합니다.
* 대체 발송 메시지를 입력하지 않으면[메시지 본문]으로 발송됩니다.

#### 사용자별 다른 메시지 설정
* <b>발신 프로필/템플릿</b>을 선택 후 <b>사용자별 다른 메시지 설정</b> 버튼을 클릭하여 하단 <b>수신자 추가</b> 탭에서 대체 발송 메시지를 설정합니다.
...
* 요청 상태를 통해 발송 요청에 대한 상태를 확인 할 수 있습니다.
* 발송 결과를 통해  발송 처리에 대한 성공/실패를 확인 할 수 있습니다.
...
* 발송 중 상태에서 <b>취소</b> 버튼을 클릭하면 일부 수신자에게 발송이 진행될 수 있습니다.
...
#### 수신자별 발송 조회
대량 알림톡 발송 건(리스트의 행) 선택하시면 하단의 리스트에서 수신번호별 알림톡 발송 내용과 발송 결과를 조회할 수 있습니다.
...
• 템플릿을 등록할 발신 프로필 또는 발신 프로필 그룹을 선택합니다. 그룹에 등록할 경우 그룹에 포함된 모든 발신 프로필이 템플릿을 사용할 수 있습니다.
...
• 템플릿 등록 시, <b>요청</b> -> <b>검수 중</b> -> <b>승인/반려</b> 상태 순서로 업데이트됩니다.
...
• 알림톡 발송이 실패될 경우, 해당 내용을 문자메시지로 대체 발송을 설정할 수 있습니다.
...
• 발신 프로필 그룹을 추가하는 데는 영업일 기준 3~4일이 소요됩니다.
