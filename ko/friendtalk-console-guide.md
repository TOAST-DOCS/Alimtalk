...
3. 전송할 내용에 광고성 정보(특가, 할인, 이벤트, 프로모션 홍보 등)가 있을 경우 광고 여부를 광고로, 없을 경우 광고 아님을 선택한 뒤 내용을 작성합니다.
...
6. 메시지 타입을 선택합니다.
    * 와이드 아이템리스트
        ...
        * 한/영 구분 없이 띄어쓰기 포함 텍스트 25자+이미지 아이템 3~4개+링크 버튼 최대 2개(가로 정렬)
    * 캐러셀
        * 최대 6개까지의 이미지와 다양한 텍스트 정보를 담을 수 있는 광고 타입의 상품입니다.
        * 한/영 구분 없이 띄어쓰기를 포함하여 제목 텍스트 20자+문구 텍스트 180자+'이미지+링크 버튼 2개(가로 정렬)'로 구성된 아이템 최대 6개
...
8. 웹 링크, 앱 링크, 봇 키워드, 메시지 전달, 상담톡 전환, 봇 전환, 비즈니스 폼 버튼을 삽입할 수 있습니다.
