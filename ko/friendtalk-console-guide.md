...
3. 전송하시는 내용에 광고성 정보(특가/할인/이벤트/프로모션 홍보 등)가 있다면, 광고 여부를 체크하시고 없다면 내용을 작성합니다.
    * 광고 메시지 대체 발송 시, 080수신 거부번호가 설정되어 있지 않으면 대체 발송 실패할 수 있습니다.

4. 발송하고 싶은 시간이 있다면 예약 발송을 선택하여 일시를 입력합니다.
    * 예약발송은 최대 1개월까지 설정 가능합니다.

5. 내용을 입력합니다.

6. 메시지 타입을 선택합니다.
    * 와이드 아이템리스트
        * 1개의 타이틀에 최소 3개에서 최대 4개까지의 리스트(이미지+아이템) 추가가 가능한 광고타입의 상품입니다.
        * 한/영 구분 없이 띄어쓰기 포함 25자 텍스트 + 이미지 아이템 3~4개 + 링크 버튼 최대 2개 (가로정렬)
    * 캐러셀
        * 최대 6개까지의 이미지와 다양한 텍스트 정보를 담을 수 있는 광고타입의 상품입니다.
        * 한/영 구분 없이 띄어쓰기 포함 '제목' 20자 텍스트 + '문구' 180자 텍스트 + '이미지 + 링크 버튼 2개(가로정렬)'로 구성된 아이템 최대 6개
...
8. 웹링크, 앱링크, 봇키워드, 메시지 전달, 상담톡 전환, 봇전환, 비즈니스폼 버튼을 삽입할 수 있습니다.
    * 기본형/캐러셀 최대 5개, 와이드 아이템리스트 최대 2개

9. 메시지 내 쿠폰을 강조해야 하는 경우 해당 버튼을 설정하여, 버튼 클릭 시 첨부한 쿠폰으로 이동할 수 있습니다.
    * 쿠폰은 기본형, 와이드 아이템리스트형만 사용 가능합니다.
...
* <b>광고 여부</b>에서 <b>광고</b>로 선택할 경우, 광고성 메시지로 간주하며 메시지 맨앞에 '(광고)' 표시, 수신 거부 방식(홈 > 친구 차단)이 표시됩니다.
...
### 이미지 등록,삭제, 조회
친구톡에 사용할 이미지를 등록, 삭제 및 등록된 이미지 정보를 확인할 수 있습니다.
...
* 비즈니스폼은 카카오톡 사용자가 손쉽게 참여할 수 있는 예약, 설문, 응모 등의 이벤트 설계를 지원하는 비즈니스 도구로 비즈니스폼을 생성하여 해당 ID를 입력합니다.
  ▶ 비즈니스폼 등록 바로가기(https://business.kakao.com/talkbizform/)
  ▶ 비즈니스폼 등록 가이드 바로가기(https://kakaobusiness.gitbook.io/main/tool/bizform)

#### 이미지 업로드 허용 범위
* 파일 형식: JPG, PNG
* 파일 크기 : 일반 [500KB 이하] / 와이드, 와이드 아이템리스트, 케러셀 [2MB 이하]
* 제한 사이즈 :
- 일반 [500px * 250px]
- 와이드 [800px * 600px]
