...
#### Feature Updates
* [API] Query Brand Message Result Updates API Added
    ...
    * The queryable period is within the last 90 days, and the date range per query is up to 31 days.

* [API] Changed Maximum Length of 080 Opt-Out Authentication Number for Brand Messages
    * The maximum allowed length of the 080 opt-out authentication number has been changed from 9 characters to 10 characters.
...
* [Console] Added a Brand Message video upload feature
    * You can directly upload and manage videos for use in Brand Message Premium Video from the console.
* [API] Added a Brand Message video upload API
    * Added an API to upload videos for Brand Message Premium Video.
* [API] Added a channel friend status field to the Brand Message result webhook
    * Added the `isAddedChannel` (Boolean) field to the Brand Message result webhook response to indicate whether the recipient is a Kakao Channel friend.
